...
// with the composable anatomy of a Card. Single- or multi-select via `multiple`;
// the indicator renders as a radio (single) or a checkbox (multiple).
//
// This is the Base UI variant. The Radix variant lives at radix/ui/choicebox and
...
//
// Anatomy (compound):
//   <Choicebox value onValueChange multiple orientation size>
//     <ChoiceboxItem value>
...
//   </Choicebox>
//
// `value` is always an array (Base UI ToggleGroup semantics): single-select is a
// group whose value holds at most one entry.
...
// Selected state is matched for BOTH primitives: Base UI emits `data-pressed`,
// Radix emits `data-[state=on]`. The cn-* tokens below are intentionally
// identical to the Radix variant — keep them in sync. The base string carries
