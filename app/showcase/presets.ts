...
// Styles we ship a registry/styles/style-*.css for. The live preview can only
// re-skin to a style whose CSS is loaded, so the Style control is limited to
// these. (Same set as AXES.style today; kept as its own name because the control
// means "loadable styles". Extend alongside the style CSS file and the build
// script's STYLES list when authoring a new style.)
