---
name: motion
description: Animate UI with Motion (the library formerly named Framer Motion, npm package `motion`) in this project's React Router v7 showcase app. Use when adding or editing any animation, transition, scroll effect, gesture (hover/tap/in-view), layout/shared-element animation, or enter/exit transition; when importing `motion`, `AnimatePresence`, `useScroll`, `useTransform`, `useReducedMotion`, etc.; or when reviewing animation code for correctness, performance, or reduced-motion support. Covers our import convention, SSR setup, the core API, performance rules, and accessibility. This is a thin in-house summary of the public Motion docs (motion.dev) — verify against the live docs for anything beyond the basics.
---

# Motion

We use **Motion** v12 (npm `motion` — this is Framer Motion, renamed). It's a root dependency (`package.json`). This skill is the project overlay; for anything not here, check the live docs at **motion.dev** (the API moves fast).
...
1. **Import from `motion/react`. Never `framer-motion`.** The old `framer-motion` package is legacy; lots of LLM/training data still uses it. Our codebase standardizes on `motion/react`.
   ...
   ```
2. **It runs under SSR.** The showcase renders on the server (`ssr: true`). In the root `vite.config.ts`, Motion is already pre-bundled via `optimizeDeps.include: ["motion", …]` — this prevents a mid-session dep re-optimization that would strand already-open tabs with a dead bundle (the file's own comment explains why). Keep `motion` in that list. If an SSR transform error ever surfaces, add `motion` to `ssr.noExternal`. No `"use client"` directive — that's a Next.js concept, not React Router.
...
`exit` only runs inside `<AnimatePresence>`, and **every direct child needs a stable, unique `key`**. A rotating-text pattern is the canonical shape:
...
```tsx
// Reveal on scroll into view — the simplest option, prefer it for showcase sections.
...
- **Type the props:** reuse `HTMLMotionProps<"div">` and `Transition` from `motion/react` for wrapper components, don't hand-roll.
